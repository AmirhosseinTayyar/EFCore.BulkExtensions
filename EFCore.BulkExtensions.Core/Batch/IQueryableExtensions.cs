...
#pragma warning restore EF1001
        ...
        if (command == null)
        {
            ...
            SelectExpression selectExpression = enumerator.Private<SelectExpression>(selectExpressionText) ??
                                                throw new InvalidOperationException(
                                                    $"{cannotGetText} {selectExpressionText}");
            IQuerySqlGeneratorFactory factory =
                enumerator.Private<IQuerySqlGeneratorFactory>(querySqlGeneratorFactoryText) ??
        ...
        {
            using var
                dbCommand = SqlAdaptersMapping.DbServer(context)
                    .QueryBuilder
                    .CreateCommand(); // Use a DbCommand to convert parameter values using ValueConverters to the correct type.
        ...
        catch (Exception ex) // Fix for BatchDelete with 'uint' param on Sqlite. TEST: RunBatchUint
        {
            ...
            if (ex.Message.StartsWith("No mapping exists from DbType") &&
                ex.Message.EndsWith(
                    "to a known SqlDbType.") || // example: "No mapping exists from DbType UInt32 to a known SqlDbType."
                ex.Message.StartsWith(npgsqlSpecParamMessage)) // Fix for BatchDelete with Contains on PostgreSQL
            {
                ...
                parameters = parameterValues.Where(pv => parameterNames.Contains(pv.Key))
                    .Select(pv =>
                        SqlAdaptersMapping.DbServer(context).QueryBuilder.CreateParameter("@" + pv.Key, pv.Value))
                    .ToList();
            }
            else
            {

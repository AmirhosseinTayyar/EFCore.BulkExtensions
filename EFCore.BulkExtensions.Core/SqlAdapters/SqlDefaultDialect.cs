...
public class SqlDefaultDialect : IQueryBuilderSpecialization
{
    ...
    public virtual List<DbParameter> ReloadSqlParameters(DbContext context, List<DbParameter> sqlParameters)
    {
        ...
        foreach (var parameter in sqlParameters)
        {
            ...
            try
            {
                ...
                {
                    sqlParameter.DbType =
                        DbType.DateTime2; // sets most specific parameter DbType possible for so that precision is not lost
                }
            }
            catch (Exception ex)
            {
                ...
                if (!ex.Message.StartsWith(noMappingText + "System.Collections.Generic.List") && // - Contains
                    ...
                    !ex.Message.StartsWith(noMappingText + typeof(JsonElement).FullName) && // - JsonElement param
                    !ex.Message.StartsWith(noMappingText + typeof(JsonDocument).FullName)) // - JsonElement param
                {
                    throw;
                }
            }
    ...
    public (string, string) GetBatchSqlReformatTableAliasAndTopStatement(string sqlQuery, SqlType databaseType)
    {
        ...
        var tableAliasEnd =
            sqlQuery[SelectStatementLength..sqlQuery.IndexOf(escapeSymbolEnd,
                StringComparison.Ordinal)]; // " TOP(10) [table_alias" / " [table_alias" : " table_alias"
        ...
        var topStatement =
            tableAliasEnd[..tableAliasStartIndex]
                .TrimStart(); // "TOP(10) " / if TOP not present in query this will be a Substring(0,0) == ""
        return (tableAlias, topStatement);
    }

    /// <inheritdoc/>
    public ExtractedTableAlias GetBatchSqlExtractTableAliasFromQuery(string fullQuery,
        string tableAlias,

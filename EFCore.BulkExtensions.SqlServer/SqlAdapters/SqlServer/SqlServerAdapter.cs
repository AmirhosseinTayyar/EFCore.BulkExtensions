...
public class SqlServerAdapter : ISqlOperationsAdapter
{
    ...
    {
        await InsertAsync(context, type, entities, tableInfo, progress, isAsync: true, cancellationToken)
            ...
            catch (Exception ex)
            {
                if (ex.Message.Contains(BulkExceptionMessage.ColumnMappingNotMatch))
                {
                    bool tableExist = isAsync
                        ? await TableInfo.CheckTableExistAsync(dbContext, tableInfo, isAsync: true, cancellationToken)
                            .ConfigureAwait(false)
                        : TableInfo.CheckTableExistAsync(dbContext, tableInfo, isAsync: false, CancellationToken.None)
                            .GetAwaiter()
                            .GetResult();

                    if (!tableExist)
                    {
                        var sqlCreateTableCopy = new SqlServerQueryBuilder().CreateTableCopy(tableInfo.FullTableName,
                            tableInfo.FullTempTableName,
                            tableInfo);
                        var sqlDropTable = new SqlServerQueryBuilder().DropTable(tableInfo.FullTempTableName,
        ...
        CancellationToken cancellationToken) where T : class
    {
        ...
        try
        {
            ...
            if (tableInfo.CreateOutputTable)
            {
                var sqlCreateOutputTableCopy = new SqlServerQueryBuilder().CreateTableCopy(tableInfo.FullTableName,
                ...
                if (operationType == OperationType.InsertOrUpdateOrDelete)
                {
                    ...
                    var sqlAlterTableColumnsToNullable =
                        SqlQueryBuilder.AlterTableColumnsToNullable(tableInfo.FullTempOutputTableName, tableInfo);
                    if (isAsync)
                    {
                        await dbContext.Database.ExecuteSqlRawAsync(sqlAlterTableColumnsToNullable, cancellationToken)
                            .ConfigureAwait(false);
                    }
                    else
                    {
            ...
            if (keepIdentity && tableInfo.HasIdentity)
            {
                ...
                {
                    await dbContext.Database.OpenConnectionAsync(cancellationToken).ConfigureAwait(false);
                    await dbContext.Database.ExecuteSqlRawAsync(sqlSetIdentityInsertTrue, cancellationToken)
                        .ConfigureAwait(false);
                }
                else
                {
            ...
            var (sql, parameters) =
                SqlQueryBuilder.MergeTable<T>(context, tableInfo, operationType, entityPropertyWithDefaultValue);
            if (isAsync)
            {
        ...
        finally
        {
            if (!tableInfo.BulkConfig.UseTempDB) // When UseTempDB is set temp tables are automaticaly dropped by Db
            {
                ...
                {
                    var sqlDropOutputTable = new SqlServerQueryBuilder().DropTable(tableInfo.FullTempOutputTableName,
                        tableInfo.BulkConfig.UseTempDB);
                    if (isAsync)
                    {
            ...
            if (identityInsertIsSet)
            {
                ...
                if (isAsync)
                {
                    await dbContext.Database.ExecuteSqlRawAsync(sqlSetIdentityInsertFalse, cancellationToken)
                        .ConfigureAwait(false);
                }
                else
                {
        ...
        CancellationToken cancellationToken) where T : class
    {
        ...
        try
        {
            ...
            var sqlCreateTableCopy =
                new SqlServerQueryBuilder().CreateTableCopy(tableInfo.FullTableName,
                    tableInfo.FullTempTableName,
                    tableInfo);
            if (isAsync)
            {
            ...
            {
                InsertAsync(context, type, entities, tableInfo, progress, isAsync: false, cancellationToken)
                    .GetAwaiter()
                    .GetResult();
            }
            ...
            tableInfo.PropertyColumnNamesDict =
                previousPropertyColumnNamesDict; // TODO Consider refactor and integrate with TimeStampPropertyName, also check for Calculated props.
            // Output only PropertisToInclude and for getting Id with SetOutputIdentity
            if (tableInfo.TimeStampPropertyName != null &&
                !tableInfo.PropertyColumnNamesDict.ContainsKey(tableInfo.TimeStampPropertyName) &&
                tableInfo.TimeStampColumnName is not null)
            {
    ...
    private static SqlBulkCopy GetSqlBulkCopy(SqlConnection sqlConnection,
        IDbContextTransaction? transaction,
        BulkConfig config)
    {
        var sqlTransaction = transaction == null ? null : (SqlTransaction) transaction.GetUnderlyingTransaction(config);
        var sqlBulkCopy = new SqlBulkCopy(sqlConnection,
            (Microsoft.Data.SqlClient.SqlBulkCopyOptions) config.SqlBulkCopyOptions,
            sqlTransaction);
        if (config.SqlBulkCopyColumnOrderHints != null)
        {
            foreach (var hint in config.SqlBulkCopyColumnOrderHints)
                sqlBulkCopy.ColumnOrderHints.Add(new Microsoft.Data.SqlClient.SqlBulkCopyColumnOrderHint(hint.Column,
                    (Microsoft.Data.SqlClient.SortOrder) hint.SortOrder));
        }
    ...
    private static void SetSqlBulkCopyConfig<T>(SqlBulkCopy sqlBulkCopy,
        TableInfo tableInfo,
        IEnumerable<T> entities,
        bool setColumnMapping,
        Action<decimal>? progress)
    {
        sqlBulkCopy.DestinationTableName =
            tableInfo.InsertToTempTable ? tableInfo.FullTempTableName : tableInfo.FullTableName;
    ...
    /// <param name="tableInfo"></param>
    /// <returns></returns>
    public static DataTable GetDataTable<T>(BulkContext context,
        Type type,
        IEnumerable<T> entities,
    ...
    /// <param name="tableInfo"></param>
    /// <returns></returns>
    private static DataTable InnerGetDataTable<T>(BulkContext context,
        ref Type type,
        IEnumerable<T> entities,
        TableInfo tableInfo)
    {
        ...
        foreach (var property in properties)
        {
            ...
            if (entityPropertiesDict.TryGetValue(property.Name, out var propertyEntityType))
            {
                ...
                if (isSqlServer && (propertyType == typeof(Geometry) || propertyType.IsSubclassOf(typeof(Geometry))))
                {
                    ...
                    if (tableInfo.BulkConfig.PropertiesToIncludeOnCompare != null)
                    {
                        throw new InvalidOperationException(
                            "OnCompare properties Config can not be set for Entity with Spatial types like 'Geometry'");
                    }
                }

                if (isSqlServer &&
                    (propertyType == typeof(HierarchyId) || propertyType.IsSubclassOf(typeof(HierarchyId))))
                {
                    propertyType = typeof(byte[]);
                }
                ...
                var omitTimeStamp = property.Name == tableInfo.TimeStampPropertyName &&
                                    tableInfo.BulkConfig.DoNotUpdateIfTimeStampChanged == false;
            ...
            else if (entityShadowFkPropertiesDict.TryGetValue(property.Name, out var fk))
            {
                ...
                if (columnName is not null && !(columnsDict.ContainsKey(columnName)) && !hasDefaultValueOnInsert)
                {
                    ...
                    columnsDict.Add(columnName, null);
                     ...
                     !tableInfo.OwnedJsonTypesDict.ContainsKey(property.Name)) // is owned
            {
                ...
                void AddOwnedType(PropertyInfo property, string prefix = "")
                {
                    ...
                    if (!tableInfo.LoadOnlyPKColumn)
                    {
                        foreach (var innerProperty in property.PropertyType.GetProperties())
                        {
                            ...
                            if (ownedEntityPropertyNameColumnNameDict.TryGetValue(innerProperty.Name,
                                    out var columnName))
                            {
                                if (tableInfo.ConvertibleColumnConverterDict.TryGetValue(propertyName,
                                        out var converter))
                                {
                                    var underlyingType = Nullable.GetUnderlyingType(converter.ProviderClrType) ??
                                ...
                                else
                                {
                                    ...
                                    if (isSqlServer &&
                                        (ownedPropertyType == typeof(Geometry) ||
                                         ownedPropertyType.IsSubclassOf(typeof(Geometry))))
                                    {
                                    ...
                                    if (isSqlServer &&
                                        (ownedPropertyType == typeof(HierarchyId) ||
                                         ownedPropertyType.IsSubclassOf(typeof(HierarchyId))))
                                    {
                                        ownedPropertyType = typeof(byte[]);
                                    }
                            ...
                            else if (tableInfo.OwnedRegularTypesDict.TryGetValue(propertyName, out var navigation) &&
                                     navigation.PropertyInfo is not null)
                            {
        ...
        foreach (T entity in entities)
        {
            ...
            foreach (var property in propertiesToLoad)
            {
                    ...
                    tableInfo.DateTime2PropertiesPrecisionLessThen7Dict.ContainsKey(property.Name))
                {
                    ...
                    if (dateTimePropertyValue is not null)
                    {
                        ...
                        long ticksToRound =
                            ...
                            (subsecondTicks % 10 == 0
                                ? 1
                                : 0); // if ends with 0 add 1 tick to make sure rounding of value .5_zeros is rounded to Upper like SqlServer is doing, not to Even as Math.Round works
                        int roundedTicks = Convert.ToInt32(Math.Round((decimal) ticksToRound / powerOf10, 0)) *
                                           powerOf10;
                        dateTimePropertyValue = dateTimePropertyValue.Value!.AddTicks(-subsecondTicks)
                            .AddTicks(roundedTicks);
                ...
                if (tableInfo.HasSpatialType && propertyValue is Geometry geometryValue)
                {
                    ...
                    {
                        sqlServerBytesWriter.IsGeography =
                            tableInfo.ColumnNamesTypesDict[tableInfo.PropertyColumnNamesDict[property.Name]] ==
                            "geography"; // "geography" type is default, otherwise it's "geometry" type
                    }
                ...
                else if (entityShadowFkPropertiesDict.ContainsKey(property.Name))
                {
                    ...
                    columnsDict[columnName] = propertyValue == null
                        ...
                        : foreignKeyShadowProperty.FindFirstPrincipal()
                            ?.PropertyInfo?.GetValue(propertyValue); // TODO Check if can be optimized
                }
                else if (entityNavigationOwnedDict.ContainsKey(property.Name) &&
                         !tableInfo.OwnedJsonTypesDict.ContainsKey(property.Name) &&
                        ...
                        foreach (var ownedProperty in property.PropertyType.GetProperties())
                        {
                            ...
                            if (ownedEntitiesMappedProperties.Contains(propertyName))
                            {
                                var ownedPropertyValue = propertyValue == null
                                    ? null
                                    : tableInfo.FastPropertyDict[propertyName].Get(propertyValue);

                                if (tableInfo.ConvertibleColumnConverterDict.TryGetValue(propertyName,
                                        out var converter))
                                {
                                    columnsDict[propertyName] = ownedPropertyValue == null
                                        ? null
                                ...
                                else if (tableInfo.HasSpatialType && ownedPropertyValue is Geometry ownedGeometryValue)
                                {
                                    ...
                                    if (tableInfo.PropertyColumnNamesDict.TryGetValue(property.Name,
                                            out string? colName))
                                    {
                                        sqlServerBytesWriter.IsGeography =
                                            tableInfo.ColumnNamesTypesDict[colName] ==
                                            "geography"; // "geography" type is default, otherwise it's "geometry" type
                                    }
                         ...
                         !tableInfo.LoadOnlyPKColumn)
                {
                    ...
                    columnsDict[columnName] =
                        (jsonPropertyValue is null) ? null : JsonSerializer.Serialize(jsonPropertyValue);
                }
            }

            if (tableInfo.BulkConfig
                .EnableShadowProperties) // TODO change for regular Shadow props to work even without this config
            {
